<template>
    <div>
        ...
        <div :key="tx.uuid" class="list_item" v-for="(tx, i) in tx_list">
            <currency-input-dropdown
                ...
                class="list_in"
            ></currency-input-dropdown>
            <button @click="removeTx(i)" class="remove_but" v-if="i !== 0 || tx_list.length > 1">
                <img src="@/assets/trash_can_dark.svg" />
            </button>
        </div>
        <button @click="addTx()" block class="add_asset" depressed v-if="showAdd">
            <fa icon="plus"></fa>
            Add Asset
        </button>
