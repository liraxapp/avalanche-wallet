...
import { getPreferredHRP as _getPreferredHRP } from 'avalanche/dist/utils'

import { Defaults, ONEAVAX, PayloadBase, PayloadTypes } from 'avalanche/dist/utils'
...
function getPreferredHRP(networkID: number | undefined = undefined) {
    if (networkID === 6) {
        return 'lirax' // If we may have other networkID:HRP pairs, we should move this to constants as an object
    } else {
        return _getPreferredHRP(networkID)
    }
}
...
function removeXTag(str: string) {
    return str.substring(2)
}

function addXTag(str: string) {
    return 'X-' + str
}
